...
trap 'rm -f /tmp/pr_*_diff.txt 2>/dev/null' EXIT
...
main() {
    ...
    # Build command with minimal permissions (read + execute for git commands)
    local cmd="codex --auto-approve-read --auto-approve-execute exec"
